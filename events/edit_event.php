...
if (!has_role('organizer')) {
    log_security_event('UNAUTHORIZED_ACCESS', 'Attempted access to edit_event without proper role');
    redirect('/index.php');
}
...
if ($event['organizer_id'] != $user_id && $user_role !== 'admin') {
    log_security_event('UNAUTHORIZED_ACCESS', "Attempted to edit event ID: $event_id without permission");
    redirect('/events/list_events.php');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!validate_csrf_token()) {
        ...
        log_security_event('CSRF_VALIDATION_FAILED', "Edit event attempt with invalid CSRF token - Event ID: $event_id");
    }
    // Validate request origin
    else if (!validate_request_origin()) {
        $error = "Invalid request origin.";
        log_security_event('INVALID_REQUEST_ORIGIN', "Edit event from suspicious origin - Event ID: $event_id");
    }
    else {
        ...
        if (empty($title) || empty($description) || empty($date) || empty($location)) {
            ...
            if ($max_participants > 0 && $max_participants < $current_registrations) {
                ...
                if ($stmt->execute()) {
                    ...
                    log_security_event('EVENT_UPDATED', "Event ID: $event_id, Title: $title");
                    ...
                    log_security_event('EVENT_UPDATE_ERROR', "Database error - Event ID: $event_id");
                }
                $stmt->close();
            }
        }
                ...
                <form method="POST" action="" class="bg-white p-4 rounded shadow">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="description" name="description" rows="4" 
                                  required minlength="10" maxlength="2000"><?php echo htmlspecialchars($event['description']); ?></textarea>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control" id="max_participants" name="max_participants"
                               value="<?php echo (int)$event['max_participants']; ?>" min="0" max="10000">
